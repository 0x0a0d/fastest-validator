				...
				if(Array.isArray(schema[fnName])){
					for (let i = 0; i < schema[fnName].length; i++) {
						...
						if(custom.type){
							ret.push( `
							// debugger; // 1
							 __temp = ${context.async ? "await " : ""}context.customFunctions[${ruleVName}.schema.${fnName}[${i}].type].call(this, __temp, ${fnCustomErrorsVName} , ${ruleVName}_${i}.schema, "${path}", parent, context, typeof parentObj !== "undefined" ? parentObj : undefined);
							`);
						}
						if(typeof custom==="function"){
							ret.push( `
							// debugger; // 2
							__temp = ${context.async ? "await " : ""}${ruleVName}.schema.${fnName}[${i}].call(this, __temp, ${fnCustomErrorsVName} , ${ruleVName}.schema, "${path}", parent, context, typeof parentObj !== "undefined" ? parentObj : undefined);
							`);
						}
					}
				}else{
					ret.push( `
					// debugger; // 3
					__temp = ${context.async ? "await " : ""}${ruleVName}.schema.${fnName}.call(this, __temp, ${fnCustomErrorsVName} , ${ruleVName}.schema, "${path}", parent, context, typeof parentObj !== "undefined" ? parentObj : undefined);
					`);
				}
