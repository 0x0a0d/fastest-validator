...
class Validator {
	 ...
	 * @return {Object} rule schema reference
	 */
		...
		if (typeof schema[fnName] == "function") {
			...
			if (this.opts.useNewCustomCheckerFunction) {
				...
				if (errors.length == 0) {
					...
					const retValue = ${context.async ? "await " : ""}${ruleVName}.schema.${fnName}.call(this, ${vName}, ${fnCustomErrorsVName} , ${ruleVName}.schema, "${path}", parent, context);
					if (${fnCustomErrorsVName}.length > 0) {
