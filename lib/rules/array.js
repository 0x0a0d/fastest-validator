...
module.exports = function ({ schema, messages }, path, context) {
	...
	if (schema.items != null) {
		...
		const innerSource = `parentObj[i] = ${context.async ? "await " : ""}context.fn[%%INDEX%%](parentObj[i], (parentField ? parentField : "") + "[" + i + "]", parent, errors, context)`;
		src.push(this.compileRule(rule, context, itemPath, innerSource, "parentObj[i]"));
		src.push(`
			}
		`);
		src.push(`
